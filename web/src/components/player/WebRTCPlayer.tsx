...
export default function WebRtcPlayer({
  ...
  const localAudioTracksRef = useRef<MediaStreamTrack[]>([]);
  const [bufferTimeout, setBufferTimeout] = useState<NodeJS.Timeout>();
  const videoLoadTimeoutRef = useRef<NodeJS.Timeout>(undefined);

  const cleanupLocalAudio = useCallback(() => {
    if (localAudioTracksRef.current.length > 0) {
      localAudioTracksRef.current.forEach((track) => {
        try {
          track.stop();
        } catch {
          // ignore
  ...
  const PeerConnection = useCallback(
    async (media: string) => {
      ...
      if (/camera|microphone/.test(media)) {
        const wantsAudio = media.indexOf("microphone") >= 0;
        const wantsVideo = media.indexOf("camera") >= 0;
        ...
        tracks.forEach((track) => {
          ...
          if (track.kind === "video") {
            ...
            localAudioTracksRef.current.push(track);
          }
        });
      }
      ...
      ws.addEventListener("message", (ev) => {
        try {
          ...
          if (msg?.type === "webrtc/candidate" && msg.value) {
            pcRef.current?.addIceCandidate({ candidate: msg.value, sdpMid: "0" }).catch(() => {});
