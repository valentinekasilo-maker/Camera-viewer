...
export default function LiveCameraView({
  ...
  // supported features & dual-stream quality management
  const availableStreams = useMemo(
    () => Object.values(camera.live.streams || {}),
    [camera.live.streams],
  );
  const mainStreamName = useMemo(() => {
    return (
      availableStreams.find((s) => !s.endsWith("_sub")) ||
      availableStreams[0] ||
      camera.name
    );
  }, [availableStreams, camera.name]);
  const subStreamName = useMemo(() => {
    return (
      availableStreams.find((s) => s.endsWith("_sub")) ||
      availableStreams[0] ||
      `${camera.name}_sub`
  ...
  const [qualityMode, setQualityMode] = useUserPersistence<string>(
    `${camera.name}-quality-mode`,
    "AUTO",
  );
  ...
  useEffect(() => {
    ...
    if (streamName != null && !availableStreams.includes(streamName)) {
      setStreamName(mainStreamName);
    }
  }, [streamNameLoaded, availableStreams, streamName, mainStreamName, setStreamName]);

  const handleQualityChange = useCallback(
    (mode: string) => {
      setQualityMode(mode);
      if (mode === "LOW" || mode === "MEDIUM") {
        setStreamName(subStreamName);
      } else {
        setStreamName(mainStreamName);
      }
    },
    [setQualityMode, setStreamName, subStreamName, mainStreamName],
  );
  ...
  const handleError = useCallback(
    (e: LivePlayerError) => {
      if (e) {
        if (streamName !== subStreamName && subStreamName) {
          toast.warning("High-quality mainstream buffering, switching to substream", {
            position: "top-center",
          });
          setStreamName(subStreamName);
        } else if (
    ...
    [config, webRTC, streamName, subStreamName, setStreamName],
  );
      ...
      >
        ...
        >
          ...
          >
            ...
            {/* Two-Way Audio / P2P Telecom */}
            <TalkControl
              cameraName={camera.name}
              supports2WayTalk={supports2WayTalk}
              cameraEnabled={cameraEnabled && !debug}
              isActive={mic}
              onToggleTalk={(active) => {
                setMic(active);
                if (active && !audio) {
            ...
            />
            {/* Camera Light / Spotlight Control */}
            <LightControl
              cameraName={camera.name}
              cameraEnabled={cameraEnabled && !debug}
            />
            ...
            {/* ANDRO-Vision Camera Intelligence AI Sheet */}
            <Sheet>
              <SheetTrigger asChild>
                <Button
                  ...
                  size="sm"
                  className={cn(
                    "flex items-center gap-1.5 px-2.5 py-1 text-xs font-semibold rounded-lg shadow-sm backdrop-blur-md border border-primary/40 bg-primary/10 text-primary hover:bg-primary/20",
                  )}
                  title="Camera Intelligence AI"
                >
                  <FaBrain className="size-3.5 text-primary animate-pulse" />
                  {isDesktop && <span>AI Intel</span>}
                </Button>
              </SheetTrigger>
              <SheetContent side="right" className="w-full sm:max-w-xl overflow-y-auto p-4 md:p-6">
                <SheetHeader className="mb-4">
                  <SheetTitle className="flex items-center gap-2 text-base font-bold">
                    <FaBrain className="text-primary size-5" />
                    ANDRO-Vision Camera Intelligence Engine
                  </SheetTitle>
                </SheetHeader>
                <CameraIntelligencePanel selectedCamera={camera.name} />
              </SheetContent>
            </Sheet>
            {/* ANDRO-Vision Stream Quality Selector */}
            {isRestreamed && (
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button
                    ...
                    size="sm"
                    className={cn(
                      "flex items-center gap-1.5 px-2.5 py-1 text-xs font-semibold uppercase tracking-wider rounded-lg shadow-sm backdrop-blur-md",
                      qualityMode === "AUTO" || qualityMode === "MAX" || qualityMode === "HIGH"
                  ...
                  >
                    ...
                    <span className="text-[10px] text-muted-foreground opacity-80">
                      {streamName?.endsWith("_sub") ? "360p" : "1080p"}
                    </span>
                  </Button>
                </DropdownMenuTrigger>
                <DropdownMenuContent align="end" className="w-48 rounded-xl p-1 text-xs">
                  <DropdownMenuItem
                    className={cn(
                      "cursor-pointer justify-between py-1.5",
                      qualityMode === "AUTO" && "bg-primary/10 font-bold text-primary",
                    )}
                    onClick={() => handleQualityChange("AUTO")}
                  >
                    <span>AUTO (Best Stream)</span>
                    <span className="text-[10px] text-muted-foreground">1080p / Adaptive</span>
                  </DropdownMenuItem>
                  <DropdownMenuItem
                    className={cn(
                      "cursor-pointer justify-between py-1.5",
                      qualityMode === "MAX" && "bg-primary/10 font-bold text-primary",
                    )}
                    onClick={() => handleQualityChange("MAX")}
                  >
                    <span>MAX QUALITY</span>
                    <span className="text-[10px] text-muted-foreground">1080p Main</span>
                  </DropdownMenuItem>
                  <DropdownMenuItem
                    className={cn(
                      "cursor-pointer justify-between py-1.5",
                      qualityMode === "HIGH" && "bg-primary/10 font-bold text-primary",
                    )}
                    onClick={() => handleQualityChange("HIGH")}
                  >
                    <span>HIGH</span>
                    <span className="text-[10px] text-muted-foreground">1080p Stream</span>
                  </DropdownMenuItem>
                  <DropdownMenuItem
                    className={cn(
                      "cursor-pointer justify-between py-1.5",
                      qualityMode === "MEDIUM" && "bg-primary/10 font-bold text-primary",
                    )}
                    onClick={() => handleQualityChange("MEDIUM")}
                  >
                    <span>MEDIUM</span>
                    <span className="text-[10px] text-muted-foreground">360p Balanced</span>
                  </DropdownMenuItem>
                  <DropdownMenuItem
                    className={cn(
                      "cursor-pointer justify-between py-1.5",
                      qualityMode === "LOW" && "bg-primary/10 font-bold text-primary",
                    )}
                    onClick={() => handleQualityChange("LOW")}
                  >
                    <span>LOW (Grid Substream)</span>
                    <span className="text-[10px] text-muted-foreground">360p Smooth</span>
                  </DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
            )}
