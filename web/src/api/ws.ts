...
function safeParse<T = unknown>(payload: unknown): T | undefined {
...
export function useFrigateEvents(): { payload: FrigateEvent } {
  ...
  const parsed = useMemo(
    () => safeParse<FrigateEvent>(payload),
    [payload],
  );
  return { payload: parsed as FrigateEvent };
}
...
export function useAudioDetections(): { payload: FrigateAudioDetections } {
  ...
  const parsed = useMemo(
    () => safeParse<FrigateAudioDetections>(payload),
    [payload],
  );
  return { payload: parsed as FrigateAudioDetections };
}
...
export function useFrigateReviews(): FrigateReview {
  ...
  return useMemo(
    () => safeParse<FrigateReview>(payload) as FrigateReview,
    [payload],
  );
}
...
export function useFrigateStats(): FrigateStats {
  ...
  return useMemo(
    () => safeParse<FrigateStats>(payload) as FrigateStats,
    [payload],
  );
}
...
export function useModelState(
  ...
  const data = useMemo(
    () => safeParse<Record<string, ModelState>>(payload),
    [payload],
  );
  ...
  return { payload: data ? data[model] : (undefined as unknown as ModelState) };
}
...
export function useEmbeddingsReindexProgress(
  ...
  const data = useMemo(
    () => safeParse<EmbeddingsReindexProgressType>(payload),
    [payload],
  );
  ...
  return { payload: data as EmbeddingsReindexProgressType };
}
...
export function useTrackedObjectUpdate(): {
  ...
  const parsed = useMemo(
    () => safeParse<TrackedObjectUpdateReturnType>(payload) ?? null,
    [payload],
  );
...
export function useTriggers(): { payload: TriggerStatus } {
  ...
  const parsed = useMemo(
    () =>
      safeParse<TriggerStatus>(payload) ?? {
        name: "",
        camera: "",
        event_id: "",
        type: "",
...
export function useJobStatus<TResults = unknown>(
  ...
  const jobData = useMemo(
    () => (safeParse<Record<string, Job<TResults>>>(payload) ?? {}),
    [payload],
  );
