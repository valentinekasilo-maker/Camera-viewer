...
const proxyHost = process.env.PROXY_HOST || "127.0.0.1:5000";
  ...
  server: {
    proxy: {
      "/live/mse/api/ws": {
        ...
        rewrite: (path) => path.replace(/^\/live\/mse\/api\/ws/, "/api/ws"),
        ws: true,
        changeOrigin: true,
      },
      "/live/webrtc/api/ws": {
        target: "ws://127.0.0.1:1984",
        rewrite: (path) => path.replace(/^\/live\/webrtc\/api\/ws/, "/api/ws"),
        ws: true,
        changeOrigin: true,
      },
      "/live/webrtc/webrtc.html": {
        target: "http://127.0.0.1:1984",
        rewrite: (path) => path.replace(/^\/live\/webrtc\/webrtc.html/, "/webrtc.html"),
        changeOrigin: true,
      },
      "/api": {
