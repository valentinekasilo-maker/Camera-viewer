...
_cached_config_dict: dict[str, Any] | None = None
_cached_config_id: int | None = None


@router.get("/config", dependencies=[Depends(allow_any_authenticated())])
def config(request: Request):
    global _cached_config_dict, _cached_config_id
    config_obj: FrigateConfig = request.app.frigate_config

    if _cached_config_dict is not None and _cached_config_id == id(config_obj):
        return JSONResponse(content=_cached_config_dict)
